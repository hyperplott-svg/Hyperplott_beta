                ...
                <AnimatePresence mode="wait">
                    {view === 'initial' && (
                        <motion.div key="initial" variants={contentVariants} initial="hidden" animate="visible" exit="exit" className="text-center">
                             <LockIcon className="mx-auto w-12 h-12 text-gray-400 mb-4" />
                            <h2 className="text-2xl font-bold text-gray-900">End-to-End Encryption</h2>
                            <p className="text-gray-600 mt-2">Your patent drafts are encrypted with zero-knowledge security. Only you can access them with your unique project key.</p>
                            <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <button onClick={() => setView('import')} className="flex flex-col items-center justify-center p-6 bg-gray-100 text-gray-800 rounded-lg font-semibold hover:bg-gray-200 hover:scale-105 transition-transform duration-200">
                                    <KeyIcon className="w-8 h-8 mb-2 text-gray-500"/>
                                    Load Existing Project
                                </button>
                                <button onClick={handleGenerate} className="flex flex-col items-center justify-center p-6 bg-gray-800 text-white rounded-lg font-semibold hover:bg-gray-900 hover:scale-105 transition-transform duration-200">
                    ...
                    {view === 'import' && (
                         <motion.div key="import" variants={contentVariants} initial="hidden" animate="visible" exit="exit">
                            ...
                            <textarea value={inputKey} onChange={e => setInputKey(e.target.value)} placeholder="Paste your saved key here" rows={4} className="w-full mt-4 bg-gray-100 p-2 rounded-md border border-gray-300 resize-none text-sm font-mono focus:outline-none focus:ring-2 focus:ring-emerald-500/50" />
                            <div className="mt-4 flex gap-4">
                                <button onClick={() => setView('initial')} className="flex-1 py-2.5 bg-gray-200 text-gray-800 rounded-lg font-semibold">Back</button>
                                <button onClick={handleImport} className="flex-1 py-2.5 bg-gray-800 text-white rounded-lg font-semibold">Unlock</button>
                            </div>
                        </motion.div>
                    )}
                    {view === 'generate' && (
                        <motion.div key="generate" variants={contentVariants} initial="hidden" animate="visible" exit="exit" className="text-left p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                            ...
                            <div className="relative mt-2">
                                <textarea readOnly value={generatedKeyStr} rows={3} className="w-full bg-white p-2 rounded-md border border-gray-300 text-xs font-mono"/>
                            ...
                            <div className="mt-4 flex items-center gap-3">
                                <input id="save-confirm" type="checkbox" checked={hasSavedKey} onChange={() => setHasSavedKey(v => !v)} className="h-4 w-4 rounded border-gray-300 text-yellow-600 focus:ring-yellow-500"/>
                                <label htmlFor="save-confirm" className="text-sm font-medium text-yellow-900 select-none">I have securely saved my key.</label>
                            </div>
                            <button onClick={handleUnlock} disabled={!hasSavedKey} className="mt-4 w-full py-2.5 bg-gray-800 text-white rounded-lg font-semibold disabled:opacity-50 disabled:cursor-not-allowed hover:bg-gray-900">
                                Unlock Drafter
                            </button>
                        </motion.div>
                    )}
