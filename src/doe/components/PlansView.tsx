                    ...
                    <AnimatePresence>
                        {billingCycle === 'yearly' && (
                            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
                                <span className="text-sm font-semibold text-primary-dark bg-primary/10 px-3 py-1 rounded-full">Save up to 16%</span>
                            </motion.div>
                        )}
                    </AnimatePresence>
                </div>
            ...
            <motion.div variants={containerVariants} initial="hidden" animate="visible" className="flex-1 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
                {plans.map((plan) => (
                    <motion.div 
                        ...
                        className={`relative flex flex-col p-6 rounded-2xl border transition-transform duration-300 hover:-translate-y-2 ${plan.isPopular ? 'border-primary/50/80 bg-primary/50/10' : 'border-black/5 bg-white/60 backdrop-blur-lg'}`}
                    >
                        {plan.isPopular && <div className="absolute top-0 -translate-y-1/2 left-1/2 -translate-x-1/2 bg-primary-dark text-white text-xs font-bold px-4 py-1 rounded-full">Most Popular</div>}
                            ...
                            <ul className="space-y-3 text-sm">
                                {plan.features.map(feature => (
                                    <li key={feature.text} className="flex items-start gap-3">
                                        {feature.included ? <CheckIcon className="w-4 h-4 mt-0.5 text-primary/50 flex-shrink-0" /> : <XIcon className="w-4 h-4 mt-0.5 text-red-500 flex-shrink-0" />}
                                        <span className={feature.included ? 'text-stone-700' : 'text-stone-400'}>{feature.text}</span>
                                    </li>
                                ))}
                            </ul>
                        </div>
                        <motion.button whileTap={{ scale: 0.95 }} className={`w-full mt-8 py-3 rounded-lg font-semibold transition-colors ${plan.isPopular ? 'bg-primary-dark hover:bg-primary-dark text-white' : 'bg-slate-800 hover:bg-[#0F172A] text-white'}`}>
                            {plan.buttonText}
                        </motion.button>
                    </motion.div>
                ))}
